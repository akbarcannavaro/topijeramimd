...
async function Botstarted() {
    ...
    const alpha = WADefault({
        ...
        browser: ['TOPI JERAMI','Safari','1.0.0'],
    ...
    for(let x of anu) {
       ...
       ppgc = 'https://telegra.ph/file/bd6b574882595e301ffff.jpg'
       }
       let wm_fatih = { url : ppgc }
       if (x.announce == true) {
       alpha.sendMessage(x.id, {image: {url: ppgc}, caption: `*ADMIN PENIS MENGUBAH PENGATURAN GRUB, HANYA ADMIN HITAM YANG DAPAT MENGIRIM PESAN*`})
       } else if (x.announce == false) {
       alpha.sendMessage(x.id, {image: {url: ppgc}, caption:`*SEKARANG MEMBER HITAM BISA MENGIRIM PESAN*`})
       } else if (x.restrict == true) {
       alpha.sendMessage(x.id, {image: {url: ppgc}, caption:`Admin Penis Mengubah pengaturan grub`})
       } else if (x.restrict == false) {
       alpha.sendMessage(x.id, {image: {url: ppgc}, caption:`Coba lu member hitam ganti profil grubnya pasti bisa`})
       } else {
       alpha.sendMessage(x.id, {image: {url: ppgc}, caption:`*Admin Penis Mengubah Nama Sirkel* *${x.subject}*`})
     }
    }
    } catch (err){
    console.log(err)
    ...
    for (let kopel of celled) {
    	...
    	let nomer = await alpha.sendTextWithMentions(kopel.from, `*${alpha.user.name}*Ajg Lo ${kopel.isVideo ? `video` : `suara`}. @${kopel.from.split('@')[0]} Gw blokir lo kontol, kalo ga mau unblok chat owner`)
        ...
        try {
            ...
            for (let num of participants) {
                try {
                    ...
                    ppuser = 'https://telegra.ph/file/a08d16c43eeca7066b4cb.jpg'
                }

                try {
                    ppgroup = await alpha.profilePictureUrl(anu.id, 'image')
                } catch {
                    ppgroup = 'https://telegra.ph/file/bd6b574882595e301ffff.jpg'
                }
                ...
                alpha.sendMessage(anu.id, {text: `${full_pesan}`})
                	...
                	alpha.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `*Welcome @${num.split("@")[0]} Di Sirkel ${metadata.subject}* 
...
${metadata.desc} `})
                  ...
                  if (isSetLeft(anu.id, set_left_db)) {            	
                      ...
                      alpha.sendMessage(anu.id, {text: `${full_pesan}`})
                       	...
                       	alpha.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `Jangan kembali ya 👋
                       	
*"Karna Setiap Ucapan Selamat Datang Akan Selalu Diakhiri Dengan Ucapan Selamat Tinggal"*

Thanks @${num.split("@")[0]} Sampai Bertemu Kembali Di Sirkel ${metadata.subject}` })
                        }
